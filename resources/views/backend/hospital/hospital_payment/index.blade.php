               ...
               <div class="kt-portlet__head-toolbar">
                  <div class="kt-portlet__head-wrapper">
                     <div class="kt-portlet__head-actions">
                        &nbsp;
                        <a href="/account/finance/payment/create" class="btn btn-brand btn-elevate btn-icon-sm">
               ...
               <table id="table_id" class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline">
                  <thead>
                     <tr>
                        ...
                        <th title="Field #4">Sub Total</th>
                        <th title="Field #4">Discount</th>
                        <th title="Field #4">Gross Total</th>
                        <th title="Field #4">Deposited Amount</th>
                  ...
                  <tbody>
                     ...
                     @foreach($payments as $payment)
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{$payment->patient != null ? $payment->patient->name : ''}}</td>
                            <td>{{$payment->doctor_id != null ? $payment->doctor->name : ''}}</td>
                            <td>{{$payment->created_at}}</td>
                            <td>{{$payment->sub_total}}</td>
                            <td>{{$payment->discount}}</td>
                            <td>{{$payment->gross_total}}</td>
                            <td>{{$payment->deposited_amount}}</td>
                            <td>
                                <span style="display: inline;">
                                    <a href="/account/finance/payment/invoice/{{$payment->id}}" class="btn btn-success btn-sm">Invoice</a>
                                    <form onsubmit="return confirm('Do you really want to delete?');" action="{{ url('/account/finance/payment', ['id' => $payment->id]) }}" style="display: inline;" method="post">
