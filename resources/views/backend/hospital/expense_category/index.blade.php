               ...
               <div class="kt-portlet__head-toolbar">
                  <div class="kt-portlet__head-wrapper">
                     <div class="kt-portlet__head-actions">
                        &nbsp;
                        <a href="/account/expenseCategory/create" class="btn btn-brand btn-elevate btn-icon-sm">
            ...
            <div class="kt-portlet__body">
               ...
               <table id="table_id" class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline">
                  ...
                  <tbody>
                     ...
                     $i = 1;
                     @endphp
                     @foreach($expense_category as $e_cat)
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{$e_cat->category}}</td>
                            <td>{{$e_cat->description}}</td>
                            <td>
                                <span style="display: inline;">
                                    <a href="/account/expenseCategory/edit/{{$e_cat->id}}" class="btn btn-success btn-sm">Edit</a>
                                    <form onsubmit="return confirm('Do you really want to delete?');" action="{{ url('/account/expenseCategory', ['id' => $e_cat->id]) }}" style="display: inline;" method="post">
