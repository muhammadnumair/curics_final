               ...
               <table id="table_id" class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline">
                  <thead>
                     <tr>
                        ...
                        <th title="Field #2">Commission Percentage</th>
                  ...
                  <tbody>
                     ...
                     @foreach(Auth::user()->clinic->doctors()->orderBy('created_at', 'DESC')->get() as $d)
                        <tr>
                            ...
                            <td>{{$d->clinic_comission}}</td>
                            <td>{{$d->doctor->appointments == null ? '0' : ($d->doctor->appointments->where('clinic_id', Auth::user()->clinic->id)->where('status', 'Treated')->sum('payable_amount'))*($d->clinic_comission/100)}}</td>
                            <td><a href="/account/hospital/finance/commission/details/{{$d->doctor->id}}" class="btn btn-success btn-sm">Details</a></td>
                        </tr>
                     @endforeach
                  </tbody>
               </table>
