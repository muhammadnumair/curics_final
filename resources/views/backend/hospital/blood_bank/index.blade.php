               ...
               <div class="kt-portlet__head-toolbar">
                  <div class="kt-portlet__head-wrapper">
                     <div class="kt-portlet__head-actions">
                        &nbsp;
                        <a href="/account/bloodBank/create" class="btn btn-brand btn-elevate btn-icon-sm">
            ...
            <div class="kt-portlet__body">
               ...
               <table id="table_id" class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline">
                  <thead>
                     <tr>
                        <th title="Field #1">#</th>
                        <th title="Field #2">Blood Group</th>
                  ...
                  <tbody>
                     ...
                     $i = 1;
                     @endphp
                     @foreach($blood_bank as $blood_b)
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{$blood_b->blood_group}}</td>
                            <td>{{$blood_b->status}}</td>
                            <td>
                                <span style="display: inline;">
                                    <a href="/account/bloodBank/edit/{{$blood_b->id}}" class="btn btn-success btn-sm">Edit</a>
                                    <form onsubmit="return confirm('Do you really want to delete?');" action="{{ url('/account/bloodBank', ['id' => $blood_b->id]) }}" style="display: inline;" method="post">
