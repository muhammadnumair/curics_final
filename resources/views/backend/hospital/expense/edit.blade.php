   ...
   <div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor mt-5">
      ...
      <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid" id="kt_content">
         <div class="kt-portlet kt-portlet--mobile">
            ...
            <form class="kt-form kt-form--label-right" action="/account/expense/update/{{$expense->id}}" method="post">
               @csrf
                <div class="kt-portlet__body">
                <div class="form-group" id="companies">
                        <label>Categories</label><br>
                        <select class="form-control m-select2" id="kt_select2_4" name="category_id" style="width: 100%;">
                           @foreach($categories as $category)
                           <option value="{{$category->id}}" {{$category->id == $expense->category_id ? 'selcted' : ''}}>{{$category->category}}</option>
                           @endforeach
                        </select>
                  </div>
                <div class="form-group">
                     <label>Amount</label>
                     <input class="form-control @error('amount') is-invalid @enderror" value="{{ old('amount', $expense->amount) }}" type="text" name="amount">
                     @error('amount')
                  ...
                  <div class="form-group">
                     ...
                     <input class="form-control @error('status') is-invalid @enderror" value="{{ old('description', $expense->description) }}" type="text" name="description">
                     @error('description')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
