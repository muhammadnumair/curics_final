               ...
               <div class="kt-portlet__head-toolbar">
                  <div class="kt-portlet__head-wrapper">
                     <div class="kt-portlet__head-actions">
                        &nbsp;
                        <a href="/account/expense/create" class="btn btn-brand btn-elevate btn-icon-sm">
               ...
               <table id="table_id" class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline">
                  <thead>
                     <tr>
                        ...
                        <th title="Field #4">Expense Amount</th>
                  ...
                  <tbody>
                     ...
                     @foreach($expenses as $q)
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{$q->description != null ? $q->description : "NILL"}}</td>
                            <td>{{$q->amount != null ? $q->amount : "NILL"}}</td>
                            <td>{{$q->category != null ? $q->category : "NILL"}}</td>
                            <td>{{$q->created_at}}</td>
                            <td>
                                <span style="display: inline;">
                                    <a href="/account/expense/edit/{{$q->id}}" class="btn btn-success btn-sm">Edit</a>
                                    <form onsubmit="return confirm('Do you really want to delete?');" action="{{ url('/account/expense', ['id' => $q->id]) }}" style="display: inline;" method="post">
