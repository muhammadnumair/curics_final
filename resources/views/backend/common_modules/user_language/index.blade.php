   ...
   <div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor mt-5">
      ...
      <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid" id="kt_content">
         <div class="kt-portlet kt-portlet--mobile">
            ...
            <form class="kt-form kt-form--label-right" action="/account/language" method="post">
               @csrf
               <div class="kt-portlet__body">
               <div class="form-group">
                     <label>Select Language</label>
                     <select class="form-control m-select2" name="language_id" id="kt_select2_1" style="width: 100%;">
                     <option value="default">Default</option>
                     @foreach($languages as $language)
                        <option value="{{$language->id}}" {{Auth::user()->language != null && Auth::user()->language->language->id === $language->id ? 'selected' : '' }}>{{$language->language_name}}</option>
                     @endforeach
                     </select>
                  </div>
               </div>
