               ...
               <table id="table_id" class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline">
                  <thead>
                     <tr>
                        ...
                        <th title="Field #7">DOB</th>
                        <th title="Field #7">Blood Group</th>
                  ...
                  <tbody>
                     ...
                     @foreach($patient as $s)
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{$s->patient->name != null ? $s->patient->name : "NILL"}}</td>
                            <td>{{$s->patient->gender != null ? $s->patient->gender : "NILL"}}</td>
                            <td>{{$s->patient->phonenum != null ? $s->patient->phonenum : "NILL"}}</td>
                            <td>{{$s->patient->dateofbirth != null ? date('d M, Y', strtotime($s->patient->dateofbirth)) : "NILL"}}</td>
                            <td>{{$s->patient->bloodgroup != null ? $s->patient->bloodgroup : "NILL"}}</td>
                            <td>
                           
                                 <span style="display: inline;">
                                    <a href="/account/patient/history/{{$s->id}}" class="btn btn-primary btn-sm">History</a>
                                    <form onsubmit="return confirm('Do you really want to delete?');" action="{{ url('/account/patient', ['id' => $s->patient->id]) }}" style="display: inline;" method="post">
                                    @method('PUT')
